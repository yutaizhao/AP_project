...
#pragma omp for nowait
        ...
        for (u64 i = 0; i < n; i++)
        {
            ...
            for (u64 j = 0; j < n; j+=8)
            {
                ...
                const f32 d8 = 1/(( (p->x[j+7] - p->x[i])* (p->x[j+7] - p->x[i])) + ((p->y[j+7] - p->y[i]) * (p->y[j+7] - p->y[i])) + (( p->z[j+7] - p->z[i]) * ( p->z[j+7] - p->z[i])) + softening * sqrtf(( (p->x[j+7] - p->x[i])* (p->x[j+7] - p->x[i])) + ((p->y[j+7] - p->y[i]) * (p->y[j+7] - p->y[i])) + (( p->z[j+7] - p->z[i]) * ( p->z[j+7] - p->z[i])) + softening));
