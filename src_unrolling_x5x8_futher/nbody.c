	    ...
	    for (u64 j = 0; j < n; j+=5)
            {
                ...
                fx += (p->x[j] - p->x[i]) * d1 + (p->x[j+1] - p->x[i]) * d2 + (p->x[j+2] - p->x[i]) * d3 + (p->x[j+3] - p->x[i]) * d4 + (p->x[j+4] - p->x[i]) * d5;
                fy += (p->y[j] - p->y[i]) * d1 + (p->y[j+1] - p->y[i]) * d2 + (p->y[j+2] - p->y[i]) * d3 + (p->y[j+3] - p->y[i]) * d4 + (p->y[j+4] - p->y[i]) * d5;
                fz += (p->z[j] - p->z[i]) * d1 + (p->z[j+1] - p->z[i]) * d2 + (p->z[j+2] - p->z[i]) * d3 + (p->z[j+3] - p->z[i]) * d4 + (p->z[j+4] - p->z[i]) * d5; 
            }
            ...
            p->vx[i] += dt * fx; // (mul, add)
            p->vy[i] += dt * fy; // (mul, add)
            p->vz[i] += dt * fz; // (mul, add)
        }
    }
